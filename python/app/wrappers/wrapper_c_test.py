...
def test_c_wrap_args_single_line(arrange_vim_buffer, mock_signature_at_cursor):
    ...
    mock_signature_at_cursor(_build_signature(2, 2, 8))
...
def test_c_wrap_args_two_lines(arrange_vim_buffer, mock_signature_at_cursor):
    buffer = arrange_vim_buffer([
        ' # c begin comment',
        '    def c_func():',
        '        c_method(',
    ...
    mock_signature_at_cursor(_build_signature(2, 3, 8))
...
def test_c_wrap_args_multiple_lines(arrange_vim_buffer, mock_signature_at_cursor):
    buffer = arrange_vim_buffer([
        ' # c begin comment',
        '    def c_func():',
        '        c_method(',
    ...
    mock_signature_at_cursor(_build_signature(2, 5, 8))
...
def test_c_wrap_args_multiple_lines_below_first(arrange_vim_buffer, mock_signature_at_cursor):
    buffer = arrange_vim_buffer([
        ' # c begin comment',
        '    def c_func():',
        '        c_method(c_a,',
    ...
    mock_signature_at_cursor(_build_signature(2, 4, 8))
...
def test_c_recognizes_c(mock_signature_at_cursor):
    mock_signature_at_cursor(_build_signature(2, 4, 0))
...
def test_c_does_not_recognize_empty_args(mock_signature_at_cursor):
    mock_signature_at_cursor(_build_signature(2, 4, 0, args=[]))
    assert wrapper_c.ArgWrapperC(4).recognized(None, None) is False
...
def test_c_does_not_recognize_other_ranges(mock_signature_at_cursor, c_row_index_diff):
    start_row_index = 5
    mock_signature_at_cursor(
        _build_signature(start_row_index, start_row_index + c_row_index_diff, 0))
    assert wrapper_c.ArgWrapperC(2).recognized(None, None) is False

def _build_signature(start_row_index,
                     end_row_index,
                     start_row_indent,
                     args=('c_a', 'c_b', 'c_c'),
                     ending=')'):
    return Signature(start_row_index=start_row_index,
