...
class WrapperSequence():
    ...
    def wrap_next(self, cursor: Cursor, buffer: VimBuffer) -> None:
    ...
    def wrap_prev(self, cursor: Cursor, buffer: VimBuffer) -> None:
        current_wrapper_index = self._get_current_wrapper_index(cursor, buffer)
        if current_wrapper_index is None:
            return
        self._get_prev_wrapper(current_wrapper_index).wrap_args(cursor, buffer)

    def _get_current_wrapper_index(self, cursor: Cursor, buffer: VimBuffer) -> Optional[int]:
